...
public class Trie {
     ...
     *
     * @param keyword The search term to add to the list of search terms.
     * @param customData The search term to add to the list of search terms.
     * @throws NullPointerException if the keyword is null.
     */
    private void addKeyword(String keyword, Object customData) {
        if (keyword.isEmpty()) {
            return;
        }
        ...
        if (customData != null) {
            addState(keyword).addEmit(keyword, customData);
        } else {
            addState(keyword).addEmit(keyword);
        }
    }
    ...
    public Emit firstMatch(final CharSequence text) {
        if (!trieConfig.isAllowOverlaps()) {
            ...
            for (int position = 0; position < text.length(); position++) {
                ...
                currentState = getState(currentState, character);
                Map<String, Object> emits = currentState.emit();

                if (emits != null && !emits.isEmpty()) {
                    for (Map.Entry<String, Object> entry : emits.entrySet()) {
               //         String emitStr = emit.getKeyword();
                        String emitStr = entry.getKey();
                        Object customeData = entry.getValue();
                        final Emit e = new Emit(position - emitStr.length() + 1, position, emitStr, customeData);
                        if (trieConfig.isOnlyWholeWords()) {
                            if (!isPartialMatch(text, e)) {
                                return e;
                            }
                        } else {
                            return e;
    ...
    private boolean storeEmits(final int position, final State currentState, final EmitHandler emitHandler) {
        ...
        final Map<String, Object> emits = currentState.emit();

        // TODO: The check for empty might be superfluous.
       if (emits != null && !emits.isEmpty()) {
           for (Map.Entry<String, Object> e : emits.entrySet()){
               String keyword = e.getKey();
               Object customeData = e.getValue();
                emitHandler.emit(new Emit(position - keyword.length() + 1, position, keyword, customeData));
                emitted = true;
            }
        }
    ...
    public static class TrieBuilder {
        ...
        /**
         * Adds a keyword with payload to the Trie's list of text search keywords.
         *
         * @param keyword The keyword to add to the list.
         * @param customData The keyword to add to the list.
         * @return This builder.
         * @throws NullPointerException if the keyword is null.
         */
        public TrieBuilder addKeyword(final String keyword, final Object customData) {
            this.trie.addKeyword(keyword, customData);
            return this;
        }
