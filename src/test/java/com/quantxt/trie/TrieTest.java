...
public class TrieTest {
    ...
    @Test
    public void testPayloadReturn() {
        Trie trie = Trie.builder().ignoreCase().onlyWholeWords()
                .addKeyword("he" , "Person")
                .addKeyword("she" , "Location")
                .addKeyword("us" , null)
                .build();
        Collection<Emit> emits = trie.parseText("he she us");
        assertEquals(3, emits.size()); // Match must not be made
        Iterator<Emit> it = emits.iterator();
        assertEquals((String) it.next().getCustomeData(), "Person");
        assertEquals((String) it.next().getCustomeData(), "Location");
    }
